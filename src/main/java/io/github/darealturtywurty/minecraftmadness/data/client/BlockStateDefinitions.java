...
public abstract class BlockStateDefinitions {
	 ...
	 * JsonDataProvider is sufficient to cause the blockstate jsons to be generated,
	 * so long as that data provider is given to the data generator, or if that data
	 * provider's act method is called by another acting data provider's act method.
	 * 
	...
	/**
	 * Creates and returns a MultipartBlockStateDefinition for whom cases can be
	 * defined. Once cases are added to the definition, giving this to a
	 * JsonDataProvider is sufficient to cause the blockstate jsons to be generated,
	 * so long as that data provider is given to the data generator, or if that data
	 * provider's act method is called by another acting data provider's act method.
	 * 
	...
	public static class VariantBlockStateDefinition {
		public static final Codec<VariantBlockStateDefinition> CODEC = Codec.unboundedMap(Codec.STRING, PartDefinition.CODEC)
				.xmap(VariantBlockStateDefinition::new, VariantBlockStateDefinition::getVariants).fieldOf("variants")
				.codec();
	...
	public static class MultipartBlockStateDefinition {
		public static final Codec<MultipartBlockStateDefinition> CODEC = CaseDefinition.CODEC.listOf()
				.xmap(MultipartBlockStateDefinition::new, MultipartBlockStateDefinition::getParts).fieldOf("multipart")
				.codec();
		...
		public static class CaseDefinition {
			public static final Codec<CaseDefinition> CODEC = RecordCodecBuilder.create(instance -> instance
					.group(Codec.unboundedMap(Codec.STRING, Codec.STRING).optionalFieldOf("when", ImmutableMap.of())
							.forGetter(CaseDefinition::getWhen),
	...
	public static class PartDefinition {
		public static final Codec<PartDefinition> CODEC = RecordCodecBuilder.create(instance -> instance
				.group(ResourceLocation.CODEC.fieldOf("model").forGetter(PartDefinition::getModel),
